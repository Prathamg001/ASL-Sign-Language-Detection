...
def Bb(list, hand_type):
    ...
    if list[4][2] < list[6][2] and list[8][2] < list[10][2] and len > 70:
        if hand_type == 'Right':
...
def again(list, hand_type):
    if (hand_type == 'Right'):
        if abs(list[21][2] - list[25][2]) < 30 and abs(list[21][2] - list[29][2]) < 30 and abs(
                list[21][2] - list[33][2]) < 30 and abs(list[21][2] - list[37][2]) < 30 and abs(
                list[21][2] - list[41][2] < 30):
            len = math.hypot(list[30][1] - list[12][1], list[30][2] - list[12][2])
            if len < 50:
                if list[5][2] < list[7][2] and list[9][2] < list[11][2] and list[13][2] < list[15][2] and list[17][2] < \
                        list[19][2]:
                    return True
    elif (hand_type == 'Left'):
        if abs(list[0][2] - list[4][2]) < 30 and abs(list[0][2] - list[8][2]) < 30 and abs(
                list[0][2] - list[12][2]) < 30 and abs(list[0][2] - list[16][2]) < 30 and abs(
                list[0][2] - list[20][2] < 30):
            len = math.hypot(list[9][1] - list[33][1], list[9][2] - list[33][2])
            if len < 50:
                if list[26][2] < list[28][2] and list[30][2] < list[32][2] and list[34][2] < list[36][2] and list[38][
                    2] < list[40][2]:
                    return True
    return False
...
def detect_gesture(landmarks, hand_type):
    if which(landmarks):
        return "Which?"
        ...
        return "Sad"
    elif landmarks[9][2] < landmarks[25][2] and again(landmarks, 'Right'):
        return "Again"
    elif landmarks[9][2] > landmarks[25][2] and again(landmarks, 'Left'):
        return "Again"
        ...
        if Aa(landmarks, 'Right'):
            return "Aa"
        elif Bb(landmarks, 'Right'):
            return "Bb"
        elif bathroom(landmarks, 'Right'):
            return "Bathroom"
...
while cap.isOpened():
    ...
    if results.left_hand_landmarks:
        landmarks = [(i, lm.x, lm.y) for i, lm in enumerate(results.left_hand_landmarks.landmark)]
        gesture = detect_gesture(landmarks, 'Left')
        cv.putText(image, gesture, (50, 50), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv.LINE_AA)
    if results.right_hand_landmarks:
        landmarks = [(i, lm.x, lm.y) for i, lm in enumerate(results.right_hand_landmarks.landmark)]
        gesture = detect_gesture(landmarks, 'Right')
        cv.putText(image, gesture, (50, 100), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv.LINE_AA)
